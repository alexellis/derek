...
func updateReleaseNotes(client *github.Client, owner, repo, latestTag string) error {
	...
	if workingReleases.PreviousTag != "" {
		releaseDiff = fmt.Sprintf("Changes between `%[1]s` and `%[2]s`: https://github.com/%[3]s/%[4]s/compare/%[1]s...%[2]s", workingReleases.PreviousTag, workingReleases.CurrentTag, owner, repo)
	} else {
		releaseDiff = fmt.Sprintf("Changes between `%[1]s` and `%[2]s`: https://github.com/%[3]s/%[4]s/compare/%[1]s...%[2]s", *includedCommits[len(includedCommits)-1].SHA, workingReleases.CurrentTag, owner, repo)
	}
